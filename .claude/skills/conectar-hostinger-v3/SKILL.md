...
# Hostinger Studio · v3 — connect · build · imagine · publish, each on demand
...
- Later, User: *"publícala"* → you **only** publish to Hostinger.
...
images? — and serve just the current ask. At the very end you may offer **one**
optional sentence naming a natural next step ("¿quieres que le genere imágenes a
medida, o la dejo así?"), but you **never start it unprompted**.
...
| "publícala / súbela / ponla online", "sube los cambios a mi hosting" | 🚀 **Publish** | `reference/13-hostinger-deploy.md` |
...
Put a finished site live using the 🔌 connection. **In the Desktop app there is no
`claude` CLI and the connector's tools are not in your session**, so the reliable
path is to **drive the connector over stdio with a tiny Node script** (full,
copy-paste script + exact tool names + gotchas in `reference/13-hostinger-deploy.md`
— read it, it's the whole method). Flow: free temp domain (or their own) → get
`order_id` → `hosting_createWebsiteV1` (async — poll `hosting_listWebsitesV1`
until the domain appears) → zip the site (index.html at root, include `.htaccess`,
stage the zip **inside the project folder**, absolute forward-slash `archivePath`)
→ `hosting_deployStaticWebsite` → **HTTP-verify the live URL**. Match tool names
**exactly** (`hosting_createWebsiteV1`, not the parked-domain/subdomains
look-alikes). **Requires only:** connector installed + the one-time browser login
(🔌). If that login hasn't happened, say so in plain Spanish and offer to do it —
